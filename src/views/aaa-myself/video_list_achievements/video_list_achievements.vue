      ...
      <el-main>
        <el-form :inline="true" :model="formInline">
          <el-form-item label="封面设计师：">
            <el-select placeholder="封面设计师" filterable>
              <el-option
                v-for="item in tabledata"
                :key="item.table_designer"
        ...
        <el-table :data="tabledata" style="width: 100%" stripe>
          ...
          />
          <el-table-column
            prop="table_article_num"
            label="稿件数量table_article_num"
            width="150px"
            align="center"
            sortable
          />
          ...
          <el-table-column
            prop="average_each_p"
            label="平均流量/篇average_each_p"
            width="150px"
            align="center"
          />
          <el-table-column
            prop="average_each_d"
            label="平均流量/日average_each_d"
            width="150px"
            align="center"
          />
          <el-table-column
          ...
          <el-table-column
            ...
            sortable
            :filters="[
              { text: '内容部门', value: 'contains' },
              { text: '海外部门', value: 'overboards' },
            ]"
          ...
          <el-table-column
            ...
            :filter-method="prioritytag"
          ><template slot-scope="scope">
            <el-tag :type="scope.row.table_manager === 'liu1' ? 'primary' : 'danger'">
        ...
        />
        <el-dialog title="修改封面" :visible.sync="cover_dialog">
          <el-form>
        ...
        <el-dialog title="配音修改" :visible.sync="audio_dialog">
          <el-form>
        ...
        <el-dialog title="剪辑修改" :visible.sync="cutter_dialog">
          <el-form>
        ...
        <el-dialog title="人工审核" :visible.sync="reviews_dialog">
          <el-table :data="reviews_table_data">
            <el-table-column label="人工审核进度" property="reviews_table_info" />
            <el-table-column
              label="审核时间"
              property="reviews_table_time"
            /><el-table-column label="审核人" property="reviews_table_name" />
            <el-table-column label="审核意见" property="reviews_table_point">
              <template slot-scope="">
                <el-button size="mini">通过</el-button>
                <br>
                <el-button size="mini">不通过</el-button>
              </template></el-table-column>
            <el-table-column label="操作" property="reviews_table_operate">
              <template>
        ...
        <el-dialog title="机器审核" :visible.sync="machine_reviews_result">
          <el-form>
            ...
            <el-form-item label="机审时间：">
              <el-input
                placeholder="2022年1月11日16:27:04，不能选择"
                disabled
                class="machine_input"
              />
            </el-form-item>
        ...
        <el-dialog title="平台选择" :visible.sync="choose_plateform">
          <el-form>
        ...
        <el-drawer title="我是更新记录" :visible.sync="drawer" :with-header="false">
          <span>我是更新记录</span>
        </el-drawer>
      </el-main>
    </el-container>
